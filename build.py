...
def builder(target_path):
	try:
		...
		sigma = 'azertyuiopq'  # you can expand this alphabet as you want to grab more hand gestures
 	 	# classes = ['palm', 'one', 'two', 'three', 'four', 'hi-five', 'fist', 'thumbs up', 'thumbs down', 'ok', 'call']
		scaler = np.array([W, H])
		accumulator = []
		...
		with mp_builder.Hands(**mp_builder_config) as detector:
			...
			while keep_processing:
				...
				status, bgr_frame = capture.read()
				if status and keep_processing:
					...
					if output:
						...
						resized_matrix = cv2.resize((matrix * 255).astype('uint8'), (W, H))
